...
function Login() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Account created. Welcome to MirrorMind!");
      navigate({ to: "/app" });
    } catch (err: any) {
      ...
      <div className="flex flex-col px-6 sm:px-12 py-10 bg-background relative">
        ...
        <div className="relative flex-1 grid place-items-center">
          ...
          >
            {mode === "signin" && (
              <div>
                ...
                <div className="mt-8 space-y-2 text-center text-xs text-muted-foreground">
                  <p>
                    New here? <button onClick={() => switchMode("signup")} className="text-cyan hover:underline">Create account</button>
                  </p>
                  <p>
                    <button onClick={() => switchMode("reset")} className="text-cyan hover:underline">Forgot password?</button>
                  </p>
                </div>
              </div>
            )}
            ...
            {mode === "signup" && (
              <div>
                <p className="text-[10px] font-mono uppercase tracking-[0.3em] text-muted-foreground mb-3">/ join atelier</p>
                <h1 className="font-serif italic text-4xl mb-2">Create account.</h1>
                <p className="text-sm text-muted-foreground mb-8">Join MirrorMind and start your AI fashion journey.</p>
                ...
                <form onSubmit={handleSignup} className="space-y-4">
                  ...
                  <div>
                    <label className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Confirm Password</label>
                    <input 
                      type="password" 
                  ...
                  <button 
                    ...
                    className="group w-full flex items-center justify-center gap-2 rounded-full bg-cyan text-background py-3 text-sm font-semibold neon-cyan disabled:opacity-50 transition"
                  >
                    {loading ? <Loader2 className="size-4 animate-spin" /> : <>Create Account <ArrowUpRight className="size-4 group-hover:translate-x-0.5 transition" /></>}
                  </button>
                </form>

                <button 
                  onClick={handleGoogleLogin}
                  className="w-full py-3 rounded-full ring-1 ring-white/10 text-sm font-medium hover:bg-white/5 transition flex items-center justify-center gap-2 mt-4"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
            ...
            {mode === "reset" && (
              <div>
                <p className="text-[10px] font-mono uppercase tracking-[0.3em] text-muted-foreground mb-3">/ recover access</p>
                <h1 className="font-serif italic text-4xl mb-2">Reset password.</h1>
                ...
                <form onSubmit={handlePasswordReset} className="space-y-4">
                  ...
                  <button 
                    ...
                    className="group w-full flex items-center justify-center gap-2 rounded-full bg-cyan text-background py-3 text-sm font-semibold neon-cyan disabled:opacity-50 transition"
                  >
                    {loading ? <Loader2 className="size-4 animate-spin" /> : <>Send Reset Link <ArrowUpRight className="size-4 group-hover:translate-x-0.5 transition" /></>}
                  </button>
                </form>
              </div>
            )}
